...
window.onload = function() {
  ...
  var svg = [
    ...
    paper.project.importSVG(
      '<path d="M134.184,753.713c60.942,32.174,133.9,39.778,199.993,20.767S458.636,710.425,494.4,650.466c44.348-74.292,54.362-168.765,109.582-235.159,50.356-60.837,129.9-87.453,206.574-102.663s156.218-21.936,227.174-54.987,133.9-100.908,133.043-180.757L94.7,106.441Z" />'
    ),
    paper.project.importSVG(
    ...
    paper.project.importSVG(
      '<path d="M381.5,667.415c9.728-52.648,52.645-93.6,100.426-115.532s100.712-28.371,152.5-37.438S739.142,493.094,781.487,461.8c36.623-26.909,62.659-65.809,90.412-102.078,28.039-35.976,60.656-71.367,103.287-85.991s97.565-1.755,117.879,39.486l-16.881,428.2Z"/>'
    ),
    paper.project.importSVG(
      '<path d="M381.5,667.415c9.728-52.648,52.645-93.6,100.426-115.532s100.712-28.371,152.5-37.438S739.142,493.094,781.487,461.8c36.623-26.909,62.659-65.809,90.412-102.078,28.039-35.976,60.656-71.367,103.287-85.991s97.565-1.755,117.879,39.486l-16.881,428.2Z"/>'
    )
  ...
  function initializePath() {
    for (var p = 0; p < path.length; p++) {
      ...
      for (var i = 0; i < path[p].segments.length; i++) {
        orgSeg[p].push([
          path[p].segments[i].point.x,
          path[p].segments[i].point.y
        ]);
      }
    }
  }
  function fixSize() {
    ...
    for (var p = 0; p < path.length; p++) {
      ...
      path[p].position = new paper.Point(
        posX + pathOptions[p].x,
        posY + pathOptions[p].y
      );
    }
  }
